...
server.get('/', (req, res) => {
    res.json({
        message: "WhatsApp API by devnolife"
    }).status(200);
});

server.get('/api/whatsapp/check-number/:number', async (req, res) => {
...
server.post('/api/whatsapp/send-message/:number/:name', async (req, res) => {
    const number = phoneNumberFormatter(req.params.number);
    const isRegistered = await client.isRegisteredUser(number);
    const message = messageFormatter(req.params.name);
    try {
        if (!isRegistered) {
            return res.status(422).json({
                code: 422,
